...
from data_finops.models import FinOpsAssessmentInput, RunState
from data_finops.persistence import JsonRunStateRepository


def state() -> RunState:
    return RunState(
        run_id="safe-run_01",
        request=FinOpsAssessmentInput(case_id="case", title="Title", description="Description"),
    )
...
def test_json_repository_retries_transient_replace_lock(tmp_path: Path) -> None:
    ...
    with patch("data_finops.persistence.os.replace", side_effect=flaky_replace):
