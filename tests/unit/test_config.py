...
from data_finops.config import load_config
...
def test_environment_override_is_typed() -> None:
    settings = load_config(
        DEFAULT_CONFIG,
        environment={
            "DATA_FINOPS__PROVIDER__MAX_RETRIES": "5",
            "DATA_FINOPS__TOOLS__ALLOW_MUTATIONS": "false",
        },
    )
...
def test_unsafe_approval_configuration_is_rejected() -> None:
    with pytest.raises(ValidationError, match="Mutations cannot be enabled"):
        load_config(
            DEFAULT_CONFIG,
            environment={
                "DATA_FINOPS__TOOLS__ALLOW_MUTATIONS": "true",
                "DATA_FINOPS__APPROVAL__MODE": "disabled",
            },
        )
